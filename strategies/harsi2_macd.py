...
class HeikinAshiRSIOscillator:
    def __init__(self):
        ...
        self.lower_x = -30
        # TP/SL parameters
        self.tp_percent = 2.5  # Take profit 1%
        self.sl_percent = 3  # Stop loss 1%
        self.tp_amount = 0.3  # Take profit amount in percentage
    ...
    def calculate_signals_with_tpsl(self, rsi, ha_open, ha_high, ha_low, ha_close, price_data, 
        ...
        for i in range(1, len(rsi)):
            ...
            X=3
            last_x_red = all((ha_open[i-X:i] > ha_close[i-X:i]).tolist())
            ...
            if i < len(ha_close_2h) and pd.notna(ha_close_2h.iloc[i]):
                ...
                ha_height_2h = ha_close_2h_curr - ha_open_2h_curr
            
            buy_condition = ha_green and ha_green_prev and not ha_green_prev2 and \
                            ha_close_curr > ha_close_prev and prev_signal != "buy" and last_x_red
...
def plot_harsi_indicator(symbol='AAPL', days=59):
    ...
    # ax2.plot(data.index, results['stoch_k'], color="#FF0000", linewidth=1, alpha=0.7, label='RSI')
    # ax2.plot(data.index, results['stoch_d'], color="#FBFF00", linewidth=1, alpha=0.7, label='RSI')
    ...
    if not valid_ha_data_2h.empty:
        harsi_2h_min = valid_ha_data_2h.min()
        harsi_2h_max = valid_ha_data_2h.max()
    ...
    if buy_count > 0:
        ...
        for idx in data.index[results['buy_signals']]:
            ...
            print(f"  {idx.strftime('%Y-%m-%d %H:%M')} - Price: ${price:.2f}")
...
if __name__ == "__main__":
    ...
    plot_harsi_indicator('RAY-USD', 59)
